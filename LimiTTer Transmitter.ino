...
/*
  Код сканирует датчик Freestyle Libre каждые 5 минут
  и отправляет текущие показания ГК в приложение xDrip+ для Android.

   Поддерживаемые датчики Libre:

  Freestyle Libre 
  Freestyle Libre2
  Freestyle Libre Pro / H

  Автоматическое определение типа датчика, замена на лету без необходимости перезапуска

 Этот скетч основан на проекте LimiTTer от JoernL, образцах скетчей модуля BM019
 из Solutions Cubed и расширений протокола, созданных savek-cc в проекте xbridge Wixel.

 Этот код заменяет исходный LimiTTer кодом @JoernL без каких-либо изменений.

 Для параметра Hardwaresource в xDrip должно быть установлено значение «Libre». Пожалуйста, используйте одну из последних версий xDrip+.

 Протокол xBridge2 используется для отправки показаний ГК в xDrip+. В случае сбоя ставится в очередь не отправленные показания ГК.
 с правильной отметкой времени и отправьте их при следующем соединении BLE. Не должно быть пропущенных показаний ГК.
 когда LimiTTer надет. Производительность батареи улучшена по сравнению с LimiTTer.

 Для использования с приложением Spike/iOS необходимо использовать протокол LimiTTer (раскомментируйте строку USE_LIMITTER_PROTOCOL)
...
#define LB_VERSION        "V1.1"        // номер версии
#define LB_MINOR_VERSION  ".02"         
#define LNAME             "LimiTTer"  // BLE имя, должно начинаться с "LimiTTer" для нормальной работы с xDrip+
...
#define RAW2DEX (8.5f)    // коэффициент масштабирования для передачи необработанных показаний libre в xDrip+/Dexcom

#define NFC_READ_RETRY 3  // количество попыток вызова read_memory() в случае ошибки чтения NFC
#define ATLNAME "AT+NAME" LNAME

#define MIN_V 3450        // пустой уровень заряда батареи
#define MAX_V 4050        // полный уровень заряда батареи
...
#define NFC_PROTOCOL_SELECT         { 0x11, 0x0D, 0x8D }
